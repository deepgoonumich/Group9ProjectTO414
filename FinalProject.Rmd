...
Answer: We created five different types of predictive models beyond the logistic model; KNN, ANN, SVM, Decision Tree, and Random Forest. The first, KNN, came with some problems. Initially, the model was able to predict perfectly, with no false positives or negatives. While on the surface this seems like a good thing, it is concerning that there is no variability, suggesting that this method is not the right tool to help us answer this question. The sample size could potentially be causing this issue. Aside from this, nearly all of the models, including the second KNN, produced accuracies between 70-75%. That is not a wide range of variability, and leads us to search for another way to differentiate between the different models. 
